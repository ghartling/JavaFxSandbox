package com.hartling.fx.controls;
 ...
 * <p>
 *
 * If the entered text matches a part of any of the supplied entries these are going to be displayed in a popup. Further the matching part of the entry is going to be displayed in a special style,
 * defined by {@link #textOccurenceStyle textOccurenceStyle}. The maximum number of displayed entries in the popup is defined by {@link #maxEntries maxEntries}.<br>
 * By default the pattern matching is not case-sensitive. This behaviour is defined by the {@link #caseSensitive caseSensitive} .
 * <p>
 *
 * The AutoCompleteTextField also has a List of {@link #filteredEntries filteredEntries} that is equal to the search results if search results are not empty, or {@link #filteredEntries
 * filteredEntries} is equal to {@link #entries entries} otherwise. If {@link #popupHidden popupHidden} is set to true no popup is going to be shown. This list can be used to bind all entries to
 * another node (a ListView for example) in the following way:
 * 
 * <pre>
 * <code>
...
public class AutoCompleteTextField<S> extends TextField {
	 ...
	 * Equal to the search results if search results are not empty, equal to {@link #entries entries} otherwise.
	 */
	...
	/**
	 * Indicates whether the Popup should be hidden or displayed. Use this if you want to filter an existing list/set (for example values of a {@link javafx.scene.control.ListView ListView}). Do this
	 * by binding {@link #getFilteredEntries() getFilteredEntries()} to the list/set.
	 */
	private boolean popupHidden = false;

	/**
	 * The CSS style that should be applied on the parts in the popup that match the entered text. <br>
	 * Default: "-fx-font-weight: bold; -fx-fill: red;"
	 * <p>
	 * Note: This style is going to be applied on an {@link javafx.scene.text.Text Text} instance. See the <i>JavaFX CSS Reference Guide</i> for available CSS Propeties.
	 */
